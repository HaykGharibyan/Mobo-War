...
type Props={receipt:BattleReceipt|null;gems:number;onHome:()=>void;onNext:()=>void;onDouble:()=>boolean;onRevive?:()=>boolean;onBoosterChoice?:(card:BoosterCard)=>void};
export function BattleResult({receipt,onHome,onNext,onDouble,onRevive,onBoosterChoice}:Props){
  ...
  const double=()=>{if(pending.current||boosted||!canAct)return;pending.current=true;setBusy(true);setMessage('');try{if(onDouble())setBoosted(true);else setMessage('Need 10 gems to double the reward.')}finally{pending.current=false;setBusy(false)}};
  const revive=()=>{if(pending.current||!onRevive)return;pending.current=true;setBusy(true);setMessage('');try{if(!onRevive())setMessage('Need 10 gems to try again.')}finally{pending.current=false;setBusy(false)}};
  ...
  return <div className={`war-result ${victory?'won':'lost'} ${challenge?'challenge-result ':''}${showBoosterChoice&&!boosterChosen?' has-booster-choice booster-pick-open ':''}${worldComplete?(nextWorld?'chapter-transition':'campaign-finale'):''}`} style={resultStyle}>
    ...
    {victory?<div className="victory-bottom">
      ...
      <div className="war-result-buttons"><button className="double-reward" disabled={busy||boosted||!canAct} onClick={double}><strong>{boosted?'✓ REWARD CLAIMED':busy?'PLEASE WAIT':'×2 REWARD'}</strong><small>{boosted?<><CurrencyIcon kind="coin" size={16}/>{receipt.coins*2} RECEIVED</>:busy?'PLEASE WAIT':<><CurrencyIcon kind="gem" size={16}/>10 GEMS · <CurrencyIcon kind="coin" size={16}/>{receipt.coins} → {receipt.coins*2}</>}</small></button>{!challenge&&<button className="war-primary" disabled={busy||!canAct} onClick={onNext}>{worldComplete?(nextWorld?'NEXT LEVEL':'RETURN TO CAMPAIGN'):'NEXT LEVEL'}</button>}<button className="home-result" disabled={busy||!canAct} onClick={onHome}>⌂ <span>HOME</span></button></div><small role="status">{message}</small>
    </div>:<div className="defeat-bottom"><div className="defeat-copy"><p>Level {receipt.level} Failed!</p><div className="defeat-earned"><CurrencyValue kind="coin" value={`+${receipt.coins}`}/><CurrencyValue kind="gem" value={`+${receipt.gems}`}/></div><small className="result-rewards-label">REWARDS</small></div><button className="defeat-callout" disabled={busy||!onRevive} onClick={revive}><strong>{busy?'PLEASE WAIT':'TRY AGAIN'}</strong><small><CurrencyIcon kind="gem" size={16}/>10 GEMS · POWER BOOST</small></button><div className="war-result-buttons"><button className="war-primary" disabled={busy} onClick={onNext}>RETRY</button><button className="home-result" disabled={busy} onClick={onHome}>⌂ <span>HOME</span></button></div><small role="status">{message}</small></div>}
