...
export type Loadout = { level:number; unitIndex:number; weaponIndex:number; unitLevel:number; weaponLevel:number; challenge?:boolean; retryBoost?:boolean; skin?:'default'|'ninja'|'knight'|'cyber'|'golden'|'blackgold'; boosters?:Record<Booster,number> };
...
// Global campaign tuning: all enemy stats are reduced by 15% from the
// campaign's existing scaled values. Keeping this in the shared scale also
// affects mini-bosses and bosses without changing their visual multipliers.
const ENEMY_GLOBAL_STRENGTH_MULTIPLIER=.85;
...
export function battleConfig(loadout:Loadout) {
  ...
  const world=getWorldByLevel(level),bossScale=WORLD_BOSS_SCALE[world.key]||1,skinBonus=SKIN_BONUS[loadout.skin||'default']||0,retryBoost=loadout.retryBoost?1.4:1;
  return {
    level, chapter:Math.ceil(level/10), world, title:world.name.toUpperCase(), bossScale,
    unit:{hp:unit.health*.55*growth*1.5*(1+skinBonus)*retryBoost,damage:unit.damage*.65*growth*2*(1+skinBonus)*(loadout.unitIndex===1?1.3:1)*retryBoost,attackSpeed:unit.attackSpeed*(loadout.retryBoost?1.1:1),moveSpeed:(34+unit.speed*6)*(1+skinBonus)*(loadout.retryBoost?1.15:1),attackRange:20},
    ...
    splash:loadout.weaponIndex===2, baseHp:Math.round(1800*scale), playerHp:Math.round(200*(loadout.retryBoost?1.25:1)), armorBonus:skinBonus, scale, enemyScale:scale*1.2*1.15*1.1*ENEMY_GLOBAL_STRENGTH_MULTIPLIER*(loadout.challenge?1.25:1),
